...
public class Stream_collect {
	...
	private static void test1() {
		...
		String cityS = citys.stream().map(x -> x.toUpperCase())
				.collect(Collectors.joining(", "));
		System.out.println(cityS);
	}

	private static void test2(List<Student> studentList) {
		Map<String, List<Student>> maps = studentList.stream()
	...
	private static void test4(List<Student> studentList) {
		Map<Boolean, List<Student>> maps = studentList.stream()
	...
	private static void test5(List<Student> studentList) {
		Optional<Student> optional1 = studentList.stream().collect(
				Collectors.maxBy(Comparator.comparing(Student::getAge)));
		optional1.ifPresent(System.out::println);

		Optional<Student> optional2 = studentList.stream().collect(
	...
	private static void test7(List<Student> studentList) {
		Map<String, String> sexNameMap = studentList.stream()
